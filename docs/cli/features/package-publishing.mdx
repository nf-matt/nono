---
title: Package Publishing Setup
description: End-to-end setup for nono packages, trusted publishing, and the GitHub Action
---

This guide is the producer-side setup for nono packages. It covers package layout, trusted publisher registration, GitHub Actions publishing with the `nono-attest` action, and the checks `nono pull` performs on the consumer side.
...
1. A package created in the registry.
2. A package namespace that matches the GitHub org that will publish it.
...
Namespace binding is enforced. If your package is `acme-corp/claude-code`, the publishing repository must be under `acme-corp/*`.

## Step 1: Create the Package Directory

A package is a signed bundle of agent-facing artifacts. Today the CLI supports these artifact types:
...
A minimal package directory might look like this:
...
- `name` should match the package name in the registry.
...
## Step 3: Create the Package in the Registry

Create the package in the registry UI first. You will need:

- Package name
- Description
- Kind
- Source GitHub repository
- Optional repo subpath if the package lives below the repo root

After the package exists, configure trusted publishing for it.
...
- Repositories outside the package namespace org
...
The reference implementation lives in the registry repository at `action/action.yml` and is named `nono-attest`.
...
- Set `path` to the package directory.
...
4. Verifies the signer repository org matches the package namespace.
5. Pins signer identity in the local package lockfile.
6. Installs verified artifacts into the local package store.
...
- The owner matches the package namespace.
