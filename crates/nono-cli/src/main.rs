...
fn run(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Pull(args) => {
            show_update_notification(&mut update_handle, cli.silent);
            package_cmd::run_pull(args)
        }
        Commands::Remove(args) => {
            show_update_notification(&mut update_handle, cli.silent);
            package_cmd::run_remove(args)
        }
        Commands::Update(args) => {
            show_update_notification(&mut update_handle, cli.silent);
            package_cmd::run_update(args)
        }
        Commands::Search(args) => {
            show_update_notification(&mut update_handle, cli.silent);
            package_cmd::run_search(args)
        }
        Commands::List(args) => {
            show_update_notification(&mut update_handle, cli.silent);
            package_cmd::run_list(args)
        }
...
fn prepare_sandbox(args: &SandboxArgs, silent: bool) -> Result<PreparedSandbox> {
    ...
    let loaded_profile = if let Some(ref profile_name) = args.profile {
        ...
        if !prof.hooks.hooks.is_empty() {
            match hooks::install_profile_hooks(Some(profile_name), &prof.hooks.hooks) {
                Ok(results) => {
